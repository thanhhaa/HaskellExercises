...
oddFactorial n
   ...
   | n == 1          = 1
   | n == 2          = 1
   | otherwise       = nNew * oddFactorial(nNew - 2)
   where nNew = if odd n then n else n-1
...
countdownHelper n str
   | n == 0    = str ++ "Liftoff!"
   | otherwise  = countdownHelper (n - 1) (str ++ show n ++ "... ")
...
smallestDivisorHelper n m
   | n `mod` m == 0 = n `div` m
   | otherwise      = smallestDivisorHelper n (m-1)
