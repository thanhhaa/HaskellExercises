...
isPalindrome str = take halfSizeForTake str == reverse (drop halfSizeForDrop str)
   where halfSizeForTake = length str `div` 2
         halfSizeForDrop = length str - halfSizeForTake
...
palindromify s = palindromify' s 0

palindromify' :: String -> Int -> String
palindromify' s count
   | isPalindrome result = result
   | otherwise      = palindromify' s (count+1)
   where start = count
         end   = length s - start
         result = substring start end s
...
greet first last = "Hello, " ++ first ++ greet' last
