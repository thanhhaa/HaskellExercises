...
firstHalf xs = take numberChar xs
    where size       = length xs
          numberChar = size `div` 2 + size `mod` 2
...
capitalize xs = unwords $ map capitalizeToUpper $ words xs

capitalizeToUpper :: String -> String
capitalizeToUpper (x:xs) = toUpper x:xs

-- better (from: haskell.mooc.fi - Haskell MOOC)
-- capitalize :: String -> String
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) (powerList k max 0)

powerList :: Int -> Int -> Int -> [Int]
powerList k max n
   | n < max    = (k^n):powerList k max (n+1)
   | otherwise  = [k^max]
...
while check update value = whileHelper check update value

whileHelper :: (a->Bool) -> (a->a) -> a -> a
whileHelper check update value
   | not (check value)        = value
   | not (check valueUpdated) = valueUpdated
   | otherwise                = whileHelper check update valueUpdated
   where valueUpdated = update value
...
whileRight check x = case check x of Left value -> value
...
joinToLength size xs = [ a++b | a <- xs, b <- xs, length (a++b) == size]

-- better (from: haskell.mooc.fi - Haskell MOOC)
-- joinToLength :: Int -> [String] -> [String]
-- joinToLength i xs = [x | x <- xs, y <- xs, let z = x + y, length z == i]
...
sumRightsNew :: [Either a Int] -> Int
sumRightsNew [] = 0
sumRightsNew (x:xs) = sum + sumRights xs
   where sum = case x of Right value -> value
                         Left value -> 0

-- Accept Challenge :D
sumRights :: [Either a Int] -> Int
sumRights = sum . map sumRightsHelper

sumRightsHelper :: Either a Int -> Int
sumRightsHelper x = case x of Left value -> 0
                              Right value -> value

-- better (from: haskell.mooc.fi - Haskell MOOC)
-- https://hackage.haskell.org/package/base-4.18.0.0/docs/Prelude.html#v:either
...
multiAppNew :: ([b1] -> b2) -> [t -> b1] -> t -> b2
multiAppNew f xs init = f $ map (\x -> x init) xs

-- Accept Challenge:
multiApp :: ([a1] -> a2) -> [t -> a1] -> t -> a2
multiApp f xs init = f (multiAppHelper xs init)

multiAppHelper :: [t -> a] -> t -> [a]
multiAppHelper [] init     = []
multiAppHelper (x:xs) init = (x init):(multiAppHelper xs init)
...
interpreter xs = interpreterH xs 0 0

interpreterH :: [String] -> Int -> Int -> [String]
interpreterH ["printY"] x y    = [show y]
interpreterH ["printX"] x y    = [show x]
interpreterH (z:zs) x y = case z of "up"      -> interpreterH zs x (y+1)
                                    "down"    -> interpreterH zs x (y-1)
                                    "printY"  -> show y : interpreterH zs x y
                                    "left"    -> interpreterH zs (x-1) y
                                    "right"   -> interpreterH zs (x+1) y
                                    "printX"  -> show x : interpreterH zs x y
                                    _         -> []
